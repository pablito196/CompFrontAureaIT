          ...
          <v-toolbar flat color="white">
            ...
            <v-dialog v-model="dialog" max-width="500px">
              <template v-slot:activator="{ on }">
                <v-btn color="success" dark class="mb-2" v-on="on">Nueva Presentación</v-btn>
              </template>
              <v-card>
                <v-card-title>
            ...
            <v-dialog v-model="adModal" max-width="350">
              <v-card>
                ...
                <v-card-text>
                  ...
                  <span v-if="adAccion==2">desactivar</span> la presentación {{adDescripcion}} ?
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
  ...
  computed: {
    formTitle() {
      if (this.dialogDelete) {
        return "Eliminar Presentación";
      } else if (this.editedIndex === -1) {
        return "Nueva Presentación";
      } else if (this.editedIndex > -1) {
        return "Editar Presentación";
      }  
    }
  },
  ...
  methods: {
    ...
    validar(){
      ...
      if(this.descripcion.length<1 || this.descripcion.length>100){
        this.validaMensaje.push('La descripción de la presentación debe tener entre 1 y 100 caracteres')
      }
    ...
    guardar(){
      ...
      if(this.editedIndex >-1){
        //codigo para editar
        axios.put('presentacion/update',{'_id':this._id, 'descripcion':this.descripcion},configuracion)
        ...
        //codigo para guardar
        axios.post('presentacion/add',{'empresa':this.$store.state.usuario.empresa,'descripcion':this.descripcion},configuracion)
        .then(function(response){
          me.limpiar();
          me.close();
          me.listar();
    ...
    activar(){
      ...
      let header = {"Token":this.$store.state.token};
      let configuracion = {headers : header};
      axios.put('presentacion/activate',{'_id':this.adId},configuracion)
        .then(function(response){
          me.adModal=0;
          me.adAccion=0;
          me.adDescripcion='';
    ...
    desactivar(){
      ...
      let header = {"Token":this.$store.state.token};
      let configuracion = {headers : header};
      axios.put('presentacion/deactivate',{'_id':this.adId},configuracion)
        .then(function(response){
          me.adModal=0;
          me.adAccion=0;
          me.adDescripcion='';
    ...
    /*remove() {
      this.presentaciones.splice(this.editedIndex, 1);
      this.close();
    }*/
  }
};
