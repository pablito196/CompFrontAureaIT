            ...
            <v-dialog v-model="adModal" max-width="290">
              <v-card>
                ...
                <v-card-text>
                  ...
                  <span v-if="adAccion==2">desactivar</span> la medida {{adDescripcion}} ?
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
  ...
  computed: {
    formTitle() {
      if (this.dialogDelete) {
        return "Eliminar Medida";
      } else if (this.editedIndex === -1) {
        return "Nueva Medida";
      } else if (this.editedIndex > -1) {
        return "Editar Medida";
      }  
    }
  },
  ...
  methods: {
    ...
    validar(){
      ...
      if(this.descripcion.length<1 || this.descripcion.length>100){
        this.validaMensaje.push('La descripción de la medida debe tener entre 1 y 100 caracteres')
      }
    ...
    guardar(){
      ...
      if(this.editedIndex >-1){
        //codigo para editar
        axios.put('medida/update',{'_id':this._id, 'descripcion':this.descripcion},configuracion)
        ...
        //codigo para guardar
        axios.post('medida/add',{'empresa':this.$store.state.usuario.empresa,'descripcion':this.descripcion},configuracion)
        .then(function(response){
          me.limpiar();
          me.close();
          me.listar();
    ...
    activar(){
      ...
      let header = {"Token":this.$store.state.token};
      let configuracion = {headers : header};
      axios.put('medida/activate',{'_id':this.adId},configuracion)
        .then(function(response){
          me.adModal=0;
          me.adAccion=0;
          me.adDescripcion='';
    ...
    desactivar(){
      ...
      let header = {"Token":this.$store.state.token};
      let configuracion = {headers : header};
      axios.put('medida/deactivate',{'_id':this.adId},configuracion)
        .then(function(response){
          me.adModal=0;
          me.adAccion=0;
          me.adDescripcion='';
    ...
    /*remove() {
      this.medidas.splice(this.editedIndex, 1);
      this.close();
    }*/
  }
};
